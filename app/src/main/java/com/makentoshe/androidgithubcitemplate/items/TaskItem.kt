        ...
        viewHolder.apply {
            ...
            if (task.date > 0) {
                ...
                if ((calendar[Calendar.DAY_OF_MONTH] == calendarCurrent[Calendar.DAY_OF_MONTH]) and (calendar[Calendar.MONTH] == calendarCurrent[Calendar.MONTH]) and (calendar[Calendar.YEAR] == calendarCurrent[Calendar.YEAR])) {
                    ...
                    text =
                        text + calendar[Calendar.DAY_OF_MONTH].toString() + " " + (calendar[Calendar.MONTH] + 1).toString() + (calendar[Calendar.YEAR]).toString()
                }
                text = text + calendar[Calendar.HOUR_OF_DAY] + ":" +  if (calendar[Calendar.MINUTE] < 10) {"0" + calendar[Calendar.MINUTE].toString()} else {calendar[Calendar.MINUTE].toString()}
                note_date.text = text
                ...
                if ((calendarCurrent.timeInMillis > calendar.timeInMillis)) {
                    ImageViewCompat.setImageTintList(
                        alarm_icon,
                    ...
                    note_title.setTextColor(0xFF888888.toInt())
                    note_date.setTextColor(0xFF888888.toInt())
                    note_background.setCardBackgroundColor(note_background.cardBackgroundColor.defaultColor + 0x11555555)
                }
            }
